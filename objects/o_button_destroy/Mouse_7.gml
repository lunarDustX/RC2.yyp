...
if (instance_exists(o_hammer)) {
	show_message("your worker is working");
	exit;
}

with (parent) {
	hammer = instance_create_layer(x, y, "Information", o_hammer);
	hammer.alarm[0] = destroyTime;
	hammer.remainingTime = destroyTime; 
	hammer.parent = self;
	if (type = building_type.house && resident.outside) {
		hammer.secretDestroy = true;
	}
}
...
if (parent.type == building_type.public) {
	...
	if (!parent.resident.outside) { // at home
		if (!WinFight(parent.resident.strength)) {
			GuardLevelUp();
			show_message("遭遇住户强烈反抗，拆除失败。引起了其他住户的警觉。");
			RiotIncrease(10);
		} else {
			show_message("强拆成功!此举引起居民们的强烈情绪。");
